...
public class ECal7 implements ActionListener,Runnable {
	JFrame f1;
...
//-----------------------------------------------------------------	
	...
	else //Calculate button
	{
		...
		else if(unit<=180)
		{
			...
			kWhCharge = ((unit-120)*7.50f)+427.5f;
			FixedCharge = 180;
			TotAmount = kWhCharge+FixedCharge;
			LPICharge = PBA*0.02f;
			TotBAmount = TotAmount+LPICharge+PBA;
		...
		else if(unit>180)
		{
			...
			kWhCharge = ((unit-180)*9.40f)+877.5f;
			FixedCharge = 240;
			TotAmount = kWhCharge+FixedCharge;
			LPICharge = PBA*0.02f;
			TotBAmount = TotAmount+LPICharge+PBA;
